...
const getSponsors = () => {
  return new Promise((resolve, reject) => {
    https
      .get('https://konstaui.com/sponsors-list.json', (resp) => {
        let data = '';
  ...
  if (tableSponsors.length > 0) {
    ...
    if (rows.length > 0 && rows[rows.length - 1].length < perRow) {
      rows[rows.length - 1].push(
        ...Array.from({ length: perRow - rows[rows.length - 1].length })
      );
    }
    tableContent = `\n<table>\n${rows
      .map((items) =>
...
const buildSponsorsList = async (sponsors) => {
  ...
  let backersContent = fs.readFileSync(
    path.resolve(__dirname, '../BACKERS.md'),
    'utf-8'
  );
...
const buildSponsors = async () => {
  ...
  if (entries) {
    const items = [...entries].filter((entry) => {
      if (
        entry.ref &&
        (entry.ref.includes('opencollective') || entry.ref.includes('patreon'))
      ) {
